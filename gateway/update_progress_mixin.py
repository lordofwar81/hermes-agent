"""Update-progress notification methods for ``GatewayRunner``.

Round 27 of the god-file decomposition. Lifted verbatim into a mixin.
Four cohesive update-notification methods moved together:

1. **Schedule watch** — ``_schedule_update_notification_watch``
   (creates the asyncio task that watches for version updates).
2. **Watch progress** — ``_watch_update_progress`` (polls for an
   in-progress self-update and reports progress to the operator chat).
3. **Send notification** — ``_send_update_notification`` (delivers a
   formatted update-available/complete notice to home channels).
4. **Inject watch notice** — ``_inject_watch_notification`` (injects
   an update notice into an in-flight message thread).

``self.*`` references resolve unchanged via the MRO. Behavior-neutral
lift matching the existing mixin pattern.

``gateway.run`` module-level globals (``logger``, ``_hermes_home``) are
lazy-imported inside each method body to avoid a circular import
(``gateway.run`` imports this mixin at module top). Stdlib (``asyncio``,
``json``, ``re``), type-only (``Platform``, ``MessageEvent``,
``MessageType``), and ``_thread_metadata_for_target`` (from
``gateway.gateway_message_pipeline``, non-circular) are imported at
module top.
"""
...
class GatewayUpdateProgressMixin:
    """Update-progress notification methods for ``GatewayRunner``."""
