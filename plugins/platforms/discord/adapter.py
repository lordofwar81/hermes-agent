...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _cancel_bot_task(self) -> None:
        ...
        if self._bot_task and not self._bot_task.done():
            # Graceful close BEFORE cancel: awaiting _client.close() lets
            # discord.py finish its teardown cleanly so the background
            # client.start() task never surfaces the _MissingSentinel
            # AttributeError race during cancel().
            if self._client:
                try:
                    await self._client.close()
                except Exception:
