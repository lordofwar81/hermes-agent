...
async def _await_with_thread_deadline(awaitable, timeout: float, *, on_abandon=None):
    ...
    # 2026-08-30 merge note: upstream consolidated this onto
    # agent.deadline.run_bounded_async (#85125), but that primitive drives its
    # deadline with a daemon threading.Timer + call_soon_threadsafe — the exact
    # pattern that deadlocks under nest_asyncio (still in the dep tree). Revisit
    # only after nest_asyncio removal.
    LOCAL FIX (2026-07-23): the upstream implementation used a daemon
    threading.Timer + loop.call_soon_threadsafe to enforce a wall-clock
    deadline. Under our gateway loop (patched by nest_asyncio via polars),
    that pattern deadlocks at initialize(). Replaced with asyncio.wait_for
    which uses standard loop timers and is nest_asyncio-safe. The on_abandon
    cleanup hook is preserved. TODO: restore upstream impl once nest_asyncio
    is removed from the dep tree. See revert commit 8310c2775.
    """
