"""Voice-channel reply / state methods for ``GatewayRunner``.

Round 19 of the god-file decomposition. Extracted verbatim into a mixin.
``self.*`` references (``self._voice_mode``, ``self._VOICE_MODE_PATH``,
``self.adapters``, ``self._recent_voice_transcripts``,
``self._reply_anchor_for_event``) resolve unchanged via the MRO. Behavior-
neutral lift matching the existing mixin pattern.
...
stateless voice helpers (``_voice_key``, ``_set_adapter_auto_tts_*``),
``_get_guild_id`` and ``_thread_metadata_for_source`` are imported at
module top — they live in already-extracted stateless modules with no
