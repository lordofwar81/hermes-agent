...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        if (
            ...
            and request_pressure_tokens > 0
            and _compressor.context_length > 0
            and request_pressure_tokens > int(_compressor.context_length * 0.90)
            and (
                getattr(_compressor, "_ineffective_compression_count", 0) >= 2
                or getattr(_compressor, "_last_compress_aborted", False)
            )
        ):
            _ctx_pct = request_pressure_tokens / _compressor.context_length * 100
            final_response = (
                "⚠️ Context is too large to continue "
                f"({request_pressure_tokens:,} tokens ≈ {_ctx_pct:.0f}% of "
