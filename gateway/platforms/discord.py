...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Interaction debounce: prevent duplicate acknowledgments (Fix #1)
        self._active_interactions: Dict[str, float] = {}
        self._interaction_ttl: float = 3.0
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async def on_message(message: DiscordMessage):
                ...
                if len(adapter_self._seen_messages) > adapter_self._SEEN_MAX:
                    ...
                    adapter_self._seen_messages = {
                        k: v
                        for k, v in adapter_self._seen_messages.items()
                        if v > cutoff
                    }
            ...
            # Wait for ready with retry logic (Fix #2: exponential backoff)
            max_retries = 5
            retry_count = 0
            while retry_count < max_retries:
                try:
                    ...
                    if retry_count >= max_retries:
                        logger.error(
                            "[%s] Timeout waiting for connection to Discord after %d retries",
                            self.name,
                            retry_count,
                            exc_info=True,
                        )
                    ...
                    logger.warning(
                        "[%s] Discord connection timeout, retry %d/%d in %ds",
                        self.name,
                        retry_count,
                        max_retries,
                        wait_time,
            ...
            logger.error(
                "[%s] Timeout waiting for connection to Discord (final)",
                self.name,
                exc_info=True,
            )
            try:
    ...
    async def _run_simple_slash(
        ...
        if interaction_id in self._active_interactions:
            logger.debug(
                "[%s] Duplicate interaction ignored: %s", self.name, interaction_id
            )
            return
        self._active_interactions[interaction_id] = now
        try:
